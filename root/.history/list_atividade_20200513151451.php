...
function check_new_opcon()
{
	...
	if($Consulta=='3')
	{
		$conexao = mysql_connect(DBHOST,DBUSER,DBPASSWORD);  
		mysql_select_db(DBNAME); 		
		$squery_users = "SELECT atividade.fk_usuario AS id_users FROM atividade WHERE atividade.fk_setor = ".  $Consulta . " AND atividade.finalizada = 0 	GROUP BY atividade.fk_usuario LIMIT 0,20";
		$result_user = mysql_query($squery_users) or die(mysql_error()); 
