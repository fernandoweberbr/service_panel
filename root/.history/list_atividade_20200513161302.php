...
function check_new_op()
{
	...
	break;
		...
		$squery_counter = "SELECT COUNT(*) FROM atividade WHERE atividade.fk_setor = ".  $Consulta . " AND atividade.finalizada = 0 ";
		$count_result = mysql_query($squery_counter) or die(mysql_error()); 
		echo  (20/$count_result);
		...
		while($users = mysql_fetch_array($result_user))
		{
			...
			if($users['id_users'])
			{
				...
				$Limit = "LIMIT 0,".round($count_result)."";
