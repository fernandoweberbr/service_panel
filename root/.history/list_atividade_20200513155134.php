...
function check_new_op()
{
	...
	break;
		...
		$squery_counter = "SELECT COUNT(id_users) atividade.fk_usuario AS id_users FROM atividade WHERE atividade.fk_setor = ".  $Consulta . " AND atividade.finalizada = 0 	GROUP BY atividade.fk_usuario ";
		$count_result = mysql_query($squery_counter) or die(mysql_error()); 
		echo  $count_result;
